...
**Date:** 2026-06-23 (original 10-source install) · 2026-07-10 (GitHub Spec Kit addition) · 2026-07-15 (official shadcn/ui skill)  
**Target:** `.claude/skills/` (project scope)  
**Total skills installed:** 322 (311 original from 10 sources + 10 from GitHub Spec Kit + 1 official shadcn/ui skill)  
...
| 12 | [shadcn/ui (official skill)](https://github.com/shadcn/ui) | 1 | ✅ Skill installed · project initialized |
...
- **Total skill count after Spec Kit: 321** (311 original + 10 Spec Kit).

---

## 12. shadcn/ui (official skill)
**Source:** `github.com/shadcn/ui` (the official shadcn/ui repository, `skills/shadcn`)  
**Installer:** the `skills` CLI  
**Installation command:**

```bash
npx skills@latest add shadcn/ui --skill shadcn --copy -y
```

**Installed to:** `.claude/skills/shadcn/` (real files, copied — not symlinked)  
**Added on:** 2026-07-15

**Skill count before → after:** **321 → 322** (+1).

The `shadcn/ui` repository ships two skills: `shadcn` (the official component/registry skill) and `migrate-radix-to-base` (a Radix→Base UI migration utility). Only **`shadcn`** was installed, matching the "official shadcn/ui skill" request and the minimal-footprint principle; `migrate-radix-to-base` was intentionally not installed.

**Footprint (all new, nothing existing modified):**
- `.claude/skills/shadcn/` — the skill (`SKILL.md`, `cli.md`, `registry.md`, `customization.md`, `mcp.md`, plus `agents/`, `assets/`, `rules/`, `evals/`).
- `.agents/skills/shadcn/` — the `skills` CLI's canonical copy store (standard CLI output).
- `skills-lock.json` — records the pinned skill and its content hash.
- **No existing skill was modified, renamed, or deleted** (verified via `git status`: only untracked additions).

### shadcn/ui project initialization

The project was initialized for shadcn/ui **without** running `shadcn init` (which would rewrite `globals.css` and `src/lib/utils.ts`). Instead a compatible `components.json` was authored by hand to match the existing project exactly, so no existing file was touched.

**`components.json` setup:**

| Field | Value | Why |
|---|---|---|
| `style` | `new-york` | Current shadcn default (resolves to `new-york-v4`) |
| `rsc` | `true` | Next.js App Router (Server Components) |
| `tsx` | `true` | TypeScript |
| `tailwind.config` | `""` | Tailwind v4 is CSS-first (no config file) |
| `tailwind.css` | `src/app/globals.css` | Existing token stylesheet |
| `tailwind.baseColor` | `neutral` | Matches the existing neutral token palette |
| `tailwind.cssVariables` | `true` | Existing tokens are CSS variables |
| `iconLibrary` | `lucide` | `lucide-react` is the project icon system |
| `aliases.ui` | `@/components/ui` | Existing components path |
| `aliases.utils` | `@/lib/utils` | Existing `cn()` utility |
| `aliases.components` / `lib` / `hooks` | `@/components` / `@/lib` / `@/hooks` | `@/*` → `src/*` |

`shadcn info` was used to confirm detection (Next.js `next-app` 16.2.10, `srcDirectory: Yes`, `rsc: Yes`, `tailwindVersion: v4`, css at `src/app/globals.css`). A `shadcn add --dry-run` confirmed the CLI reads `components.json` correctly (it resolved `styles/new-york-v4/dialog.json`).

### Components added — installed per project (intentional)

**The shadcn skill and `components.json` are fully installed and ready. No individual UI components are bundled in this master template, by design.**

Individual shadcn components are installed **per project**, when a project actually needs them, with:

```bash
npx shadcn@latest add <component>
```

Keeping the master template component-free avoids shipping primitives every project would then have to prune, and lets each project pull exactly the set its spec calls for (e.g. `dialog`, `sheet`, `dropdown-menu`, `tooltip`, `tabs`, `form`, `input`, `textarea`, `label`, `select`, `sonner`, or anything else in the registry).

**Network note:** `ui.shadcn.com` access is only required at the moment components are *fetched* (`shadcn add`). Using the installed skill itself — its guidance, `cli.md`/`registry.md` references, `shadcn info`, and reading `components.json` — needs no network access. Skill installation and `npm` rely on `github.com` and `registry.npmjs.org`, which are independent of `ui.shadcn.com`. In environments whose egress policy does not allow `ui.shadcn.com`, allowlist it before running `shadcn add`.

The existing custom `button`, `accordion`, `card`, and layout components remain the starting point; shadcn versions of those are added per project only when they improve accessibility or composition.
