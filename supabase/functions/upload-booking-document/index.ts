...
Deno.serve(async (req: Request) => {
  ...
  const documentType = String(form.get("document_type") ?? "aadhaar").trim().slice(0, 40) || "aadhaar";
  ...
  const { data: tokenRow, error: tokenError } = await client.from("booking_document_upload_tokens").select("id,stay_request_id,expires_at,max_uploads,used_uploads,revoked_at").eq("token_hash", tokenHash).is("revoked_at", null).gt("expires_at", now).maybeSingle();
  ...
  const { data: doc, error: insertError } = await client.from("booking_documents").insert({ stay_request_id: stay.id, guest_id: guestId, document_type: documentType, storage_provider: "supabase", storage_key: storageKey, original_filename: file.name.slice(0, 255), mime_type: file.type, size_bytes: file.size }).select("id,original_filename,uploaded_at").single();
  if (insertError) { await client.storage.from("booking-documents-private").remove([storageKey]); return json({ error: "Document record could not be saved" }, 500); }
  await client.from("booking_document_upload_tokens").update({ used_uploads: tokenRow.used_uploads + 1 }).eq("id", tokenRow.id).eq("used_uploads", tokenRow.used_uploads);
