...
Deno.serve(async(req:Request)=>{
  ...
  if(!supabaseUrl||!serviceKey||!turnstileSecret){console.error("public-form config missing",{supabaseUrl:Boolean(supabaseUrl),serviceKey:Boolean(serviceKey),turnstileSecret:Boolean(turnstileSecret)});return json({error:"Public form verification is not configured"},503);}
  ...
  if(!validatePayload(formType,payload)){console.warn("public-form invalid payload",{formType});return json({error:"Invalid submission"},422);}
  ...
  console.log("turnstile verify",{formType,httpOk:verifyResponse.ok,success:Boolean(verified.success),action:verified.action??null,hostname:verified.hostname??null,errorCodes:verified["error-codes"]??[]});
  ...
  if(allowedHostnames.length&&(!verified.hostname||!allowedHostnames.includes(verified.hostname.toLowerCase()))){console.warn("turnstile hostname rejected",{hostname:verified.hostname??null,allowedHostnames});return json({error:"Verification hostname rejected"},403);}
