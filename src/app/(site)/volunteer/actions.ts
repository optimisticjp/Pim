...
export async function submitVolunteerAction(formData:FormData):Promise<never>{
  ...
  let number="";try{const rows=await supabasePublicRpc<Array<{application_id:string;application_number:string}>>("submit_volunteer_application",{payload});number=rows[0]?.application_number??"";}catch{redirect("/volunteer?error=submit");}
