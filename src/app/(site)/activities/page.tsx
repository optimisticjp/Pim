...
import { prototypeSevaUpdates } from "@/lib/prototype-content";
...
const participationChoices = [
  ...
  { title: "નજીકના આશ્રમ સાથે જોડાણ", copy: "પસંદગીનો આશ્રમ જણાવો અથવા પસંદગી ખાલી રાખો. સત્તાવાર જોડાણ માટે આશ્રમ સૂચિમાં ચકાસેલી સંપર્ક વિગતો જુઓ." },
];
...
export default function ActivitiesPage() {
  return <>
    ...
    <section className="section-pad"><div className="container-site"><SectionHeading eyebrow="સેવાની ઝલક" title="આશ્રમ જીવનમાં વ્યક્ત થતો સેવાભાવ" description="વિવિધ સેવા ક્ષેત્રો કેવી રીતે સમૂહ જોડાણ અને જવાબદારીનું સ્વરૂપ લે છે તેની પસંદ કરેલી ઝલક."/><div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{prototypeSevaUpdates.map(([title,copy],index)=><article key={title} className={`rounded-[1.25rem] border p-5 ${index===0?"border-sacred-green/25 bg-[#e8eee8]":"border-border bg-surface"}`}><p className="text-xs font-bold text-gold-deep">સેવા પ્રવૃત્તિ</p><h3 className="mt-3 font-serif text-xl font-bold text-primary">{title}</h3><p className="mt-2 text-sm leading-7 text-muted-foreground">{copy}</p></article>)}</div></div></section>
