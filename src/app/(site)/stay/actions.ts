...
export async function submitStayRequestAction(formData:FormData):Promise<never>{
  ...
  let number="";try{const rows=await supabasePublicRpc<Array<{request_id:string;request_number:string}>>("submit_stay_request",{payload});number=rows[0]?.request_number??"";}catch{redirect("/stay?error=submit");}
