...
const str=(fd:FormData,n:string,max=6000)=>{const v=String(fd.get(n)??"").trim();return v?v.slice(0,max):null};
const bool=(fd:FormData,n:string)=>fd.get(n)==="on"||fd.get(n)==="true";
const num=(fd:FormData,n:string,def=100)=>{const v=Number(str(fd,n,10));return Number.isFinite(v)?v:def};
const permissions:Record<string,string>={guru_profiles:"guru.manage",guru_chapters:"guru.manage",heritage_documents:"heritage.manage",media_folders:"media.manage",media_assets:"media.manage"};
async function ctx(permission:string){const s=await requireAdminSession();if(!hasAdminPermission(s,permission)&&!s.profile?.is_super_admin)throw new Error("Permission denied");const token=await getAdminAccessToken();if(!token)redirect("/admin/login");return{s,token};}
...
export async function permanentDeleteCmsRecordAction(fd:FormData){const s=await requireAdminSession();if(!s.profile?.is_super_admin)throw new Error("Super Admin required");const table=str(fd,"table",80),id=str(fd,"id",40);if(!table||!id||!permissions[table])return;const token=await getAdminAccessToken();if(!token)redirect("/admin/login");await supabaseRest(`${table}?id=eq.${encodeURIComponent(id)}`,token,{method:"DELETE",prefer:"return=minimal"});refresh();}
