  ...
  return (
    <footer className="border-t border-white/10 bg-[#32151b] pb-24 text-[#f7eadc] md:pb-0">
      <div className="container-site grid gap-10 py-14 lg:grid-cols-[1.2fr_.8fr_.8fr_.9fr] lg:gap-12">
        ...
        <div><h2 className="text-base font-bold text-white">મુખ્ય આશ્રમ</h2><div className="mt-4 space-y-3 text-[14px] leading-6 text-[#d9cbbf]">{mainAshram ? <>{address ? <p className="flex gap-2"><MapPin className="mt-1 h-4 w-4 shrink-0 text-[#e4ae58]" />{address}</p> : null}{phone ? <a href={`tel:${phone.replace(/\s+/g, "")}`} className="flex items-center gap-2 hover:text-white"><Phone className="h-4 w-4 text-[#e4ae58]" />{phone}</a> : null}</> : <Link href="/ashrams" className="flex items-center gap-2 hover:text-white"><MapPin className="h-4 w-4 text-[#e4ae58]" />આશ્રમ ડિરેક્ટરી જુઓ</Link>}<a href={youtubeChannel} target="_blank" rel="noreferrer" className="flex items-center gap-2 hover:text-white"><YouTubeMark className="h-4 w-4 text-[#e4ae58]" />YouTube સત્સંગ <ExternalLink className="h-3.5 w-3.5" /></a></div></div>
