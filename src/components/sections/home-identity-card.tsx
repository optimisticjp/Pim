...
export function HomeIdentityCard({ variant = "hero", className }: { variant?: "hero" | "compact"; className?: string }) {
  ...
  return <div className={cn(compact ? "rounded-[1.4rem]" : "sacred-arch", "relative overflow-hidden border border-primary/15 bg-primary text-primary-foreground shadow-[0_20px_55px_rgba(79,37,31,.17)]", className)}>
    ...
    <div className={cn("relative text-center", compact ? "grid grid-cols-[auto_1fr] items-center gap-x-4 gap-y-3 p-5 text-left" : "px-5 pb-6 pt-20 sm:px-8 sm:pb-8 sm:pt-24")}>
      ...
      {compact && <Link href="/parampara" className="col-span-2 inline-flex min-h-11 w-full items-center justify-between rounded-xl border border-white/15 bg-[#fff8ec] px-4 text-sm font-bold text-primary">ગુરુપરંપરા જાણો <ArrowRight className="h-4 w-4" /></Link>}
    </div>
  </div>;
}
